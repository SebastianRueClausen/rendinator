...
vec3 cast_ray(const vec3 ray_dir, const vec3 sun_dir) {
	const vec3 origin = vec3(0.0, -100.0, 0.0);
	const vec3 irradiance = dir_light.irradiance.xyz * 0.6;

	return integrate_scattering(origin, ray_dir, sun_dir, irradiance);
}
