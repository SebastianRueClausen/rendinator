...
layout (std430, set = 0, binding = 1) readonly uniform LightInfoBuf {
...
vec3 sky_color(const vec3 ray_dir, const vec3 sun_dir) {
	...
	const vec3 irradiance = light_info.dir_light.irradiance.xyz;
...
void main() {
	...
	vec3 sun_dir = light_info.dir_light.dir.xyz;
	sun_dir.y = -sun_dir.y;
